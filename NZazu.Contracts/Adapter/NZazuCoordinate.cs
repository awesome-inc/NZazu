...
namespace NZazu.Contracts.Adapter
{
    ...
    public class NZazuCoordinate
    {
        ...
        public bool GetIsValid() => Lat >= LatMin && Lat <= LatMax &&
        ...
        public static NZazuCoordinate Parse(string latLonDecimalWithSpace)
        {
            ...
            var canLat = double.TryParse(split[0], NumberStyles.AllowDecimalPoint | NumberStyles.AllowLeadingSign, NumberFormatInfo.InvariantInfo, out double lat);
            var canLon = double.TryParse(split[1], NumberStyles.AllowDecimalPoint | NumberStyles.AllowLeadingSign, NumberFormatInfo.InvariantInfo, out double lon);
